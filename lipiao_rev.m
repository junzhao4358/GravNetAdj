function  asd = lipiao_rev(xdobs,deg,getr,gutu,yiqigai,qiya,kk,sst,app_SolidTide,app_AtmosphericPressure,app_InstrumentHight,app.OceanTideCheckBox.Value)         
%零漂率计算和零漂改正
yiqi_name = xdobs(1,1);
yiqi_num = 0; st_num = 0;
...
%      if app.InstrumentHightCheckBox.Value
      ...
      if app.OceanTideCheckBox.Value
         gRC1  = gRC1  + ocean_load(i) ;
         gRC3  = gRC3  + ocean_load(i+2);
         gRC2  = gRC2  + ocean_load(i+1); 
      end
      ...
      %存储数据
      adj(st_num,:) = [xdobs{i,1},xdobs(i,4:6),xdobs{i,2},xdobs{i+1,2},xdobs{i,8},xdobs{i+1,8},gRCend1,gRCend2,duancha, k,xdobs{i,26},1];
    elseif sst(st_num) == 4
           %零漂率计算:采用A-B-B-A测量模式
           %计算各项改正后观测值
